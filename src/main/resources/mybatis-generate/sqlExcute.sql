...
CREATE TABLE `job_info_detail` (
  ...
  `job_company` TEXT NOT NULL DEFAULT '' COMMENT '职位发布公司',
  `job_company_descrip` varchar(255) NOT NULL DEFAULT '' COMMENT '职位发布公司简介',
  ...
  `job_descrip` TEXT NOT NULL DEFAULT '' COMMENT '职位详细描述',
  `add_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `del_flag` tinyint(4) NOT NULL DEFAULT '0' COMMENT '删除标记',
  `remark` varchar(50) NOT NULL DEFAULT '' COMMENT '备注',
